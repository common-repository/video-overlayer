<?php
/**
 * Builds the Video Overlayer Options functions.
 *
 * @package Video Overlayer
 */
 
/**
 * Get the latest video_overlayer_settings array from the database
 * and then cache it, if not otherwise specified, so specific
 * Video Overlayer Settings values (or the entire array) can be efficiently accessed.
 *
 * @since 1.0
 * @return either the entire video_overlayer_settings array or a specific key/value.
 */
function video_overlayer_get_settings( $key, $args = false ) {
	...
	if ( $args ) {
		...
		// then update the $options_cache variable with the latest copy of the video_overlayer_settings array.
		if ( empty( $options_cache ) || ! $args['cached'] ) {
			$options_cache = get_option( 'video_overlayer_settings' );
		}
		// If the $args['array'] key is not false then return the entire
		// video_overlayer_settings array through the $options_cache variable.
		if ( $args['array'] ) {
	...
	// latest copy of the video_overlayer_settings array and set the $options_set variable to true.
	else {
		$options_cache = get_option( 'video_overlayer_settings' );
		$options_set = true;
	}
...
//end lib/functions/cobalt-options.php